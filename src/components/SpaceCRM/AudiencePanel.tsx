...
import { enrichSpeakerData, XUserProfile, XTweet } from '../../services/x.service';
...
const MOCK_ATTENDEES = Array(20).fill(null).map((_, i) => ({
  ...
  joinedDate: new Date(Date.now() - Math.floor(Math.random() * 1000 * 86400000)).toISOString(),
}));

// All possible interests for filtering
const ALL_INTERESTS = ['AI', 'Web3', 'Marketing', 'Technology', 'Design', 'Business', 'Finance', 'Crypto'];

// All possible locations for filtering
const ALL_LOCATIONS = ['New York', 'San Francisco', 'London', 'Berlin', 'Tokyo', 'Remote'];
 ...
 */
const AudiencePanel: React.FC<AudiencePanelProps> = ({ onSelectAttendees, space }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedAttendees, setSelectedAttendees] = useState<string[]>([]);
  const [isFilterDrawerOpen, setIsFilterDrawerOpen] = useState(false);
  const [attendeeDetailDrawer, setAttendeeDetailDrawer] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<'speakers' | 'attendees'>('speakers');
  ...
  const [enrichedSpeakers, setEnrichedSpeakers] = useState<(User & { xProfile?: XUserProfile; recentTweets?: XTweet[] })[]>([]);
  ...
  const fetchEnrichedSpeakers = async (speakersToEnrich: User[]) => {
    ...
    for (const speaker of speakersToEnrich) {
      try {
        console.log(`Attempting to enrich speaker: ${speaker.twitter_screen_name}`);
        const enriched = await enrichSpeakerData(speaker);
        if (enriched) {
          console.log(`Successfully enriched speaker: ${speaker.twitter_screen_name}`);
          newlyEnrichedData.push(enriched);
        } else {
          console.log(`Failed to enrich speaker (enrichSpeakerData returned null): ${speaker.twitter_screen_name}`);
          // Push original speaker data if enrichment fails but we still want to show the user
          newlyEnrichedData.push({ ...speaker, xProfile: undefined, xTweets: [] });
        }
        ...
        console.error(`Error enriching speaker ${speaker.twitter_screen_name}:`, error);
  ...
  useEffect(() => {
    ...
    if (space?.speakers && space.speakers.length > 0 && enrichedSpeakers.length === 0) {
      console.log(`Space data updated, fetching enriched speaker data for the FIRST speaker only...`); 
      ...
      console.log('No speakers in space data or space cleared, clearing enriched speakers.');
      setEnrichedSpeakers([]);
      setIsLoadingProfiles(false); 
    }
  ...
  const filteredSpeakers = space?.speakers.map(speaker => ({
    ...speaker,
    xProfile: enrichedSpeakers.find(es => es.user_id === speaker.user_id)?.xProfile
  })).filter(speaker => {
  ...
  const filteredAttendees = MOCK_ATTENDEES.filter(attendee => {
    // Search term filter
    if (searchTerm && !attendee.displayName.toLowerCase().includes(searchTerm.toLowerCase()) && 
    ...
    // Followers filter
    if (filterFollowers === 'large' && attendee.followersCount < 5000) return false;
    if (filterFollowers === 'medium' && 
        (attendee.followersCount < 1000 || attendee.followersCount >= 5000)) return false;
    if (filterFollowers === 'small' && attendee.followersCount >= 1000) return false;
    
    // Location filter
    if (filterLocation !== 'all' && attendee.location !== filterLocation) return false;
      ...
      <Paper sx={{ 
        ...
        <List sx={{ 
          ...
          {activeTab === 'speakers' ? (
                    ...
                    {filteredSpeakers.map((speaker) => (
                      <ListItem 
                        ...
                        secondaryAction={
                          <IconButton 
                            edge="end" 
                            onClick={() => handleAttendeeSelect(speaker.user_id)}
                            color={selectedAttendees.includes(speaker.user_id) ? 'primary' : 'default'}
                          >
                            <BookmarkIcon />
                          </IconButton>
                        }
                      ...
                      >
                        <ListItemAvatar>
                          <Badge
                            overlap="circular"
                            badgeContent={speaker.xProfile?.verified ? "✓" : undefined}
                            color="primary"
                          >
                            <Avatar 
                              src={speaker.xProfile?.profile_image_url || speaker.avatar_url} 
            ...
            <Box>
              <Typography variant="h6" sx={{ mb: 2 }}>Listeners</Typography>
              <Box sx={{ 
                maxHeight: 'calc(100vh - 500px)', // Adjust this value based on your layout
                overflowY: 'auto',
                pb: 4, // Add bottom padding
          ...
          {((activeTab === 'speakers' && enrichedSpeakers.length === 0) || 
            ...
            <Box sx={{ p: 3, textAlign: 'center' }}>
              <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                No {activeTab === 'speakers' ? 'speakers' : 'attendees'} match your filters
              </Typography>
            </Box>
          )}
        </List>
      ...
      >
        <Box sx={{ p: 2 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
            <Typography variant="h6">Filter {activeTab === 'speakers' ? 'Speakers' : 'Attendees'}</Typography>
          ...
          {activeTab === 'attendees' && (
            <>
              ...
              <Typography variant="subtitle2" sx={{ mb: 2 }}>Interests</Typography>
          ...
          <Box sx={{ p: 2 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
              ...
              <Box>
                ...
                {activeTab === 'attendees' && (
                  <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                    {(selectedAttendee as any).followersCount.toLocaleString()} followers
                  </Typography>
                )}
              </Box>
            </Box>
            ...
            {detailTab === 'profile' && (
              <>
                {activeTab === 'speakers' ? (
                  <>
                    <Typography variant="subtitle1" sx={{ mb: 1 }}>Bio</Typography>
                    <Typography variant="body2" sx={{ mb: 3 }}>
                      {(selectedAttendee as User & { xProfile?: XUserProfile }).xProfile?.description || 'No bio available'}
                    </Typography>
                    
                    {(selectedAttendee as User & { xProfile?: XUserProfile }).xProfile?.public_metrics && (
                      <>
                        <Typography variant="subtitle1" sx={{ mb: 1 }}>X Stats</Typography>
                        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, mb: 3 }}>
                          <Box>
                            <Typography variant="body2" sx={{ color: 'text.secondary' }}>Followers</Typography>
                            <Typography variant="body1">
                              {(selectedAttendee as User & { xProfile?: XUserProfile }).xProfile?.public_metrics.followers_count.toLocaleString()}
                            </Typography>
                          </Box>
                          <Box>
                            <Typography variant="body2" sx={{ color: 'text.secondary' }}>Following</Typography>
                            <Typography variant="body1">
                              {(selectedAttendee as User & { xProfile?: XUserProfile }).xProfile?.public_metrics.following_count.toLocaleString()}
                            </Typography>
                          </Box>
                          <Box>
                            <Typography variant="body2" sx={{ color: 'text.secondary' }}>Tweets</Typography>
                            <Typography variant="body1">
                              {(selectedAttendee as User & { xProfile?: XUserProfile }).xProfile?.public_metrics.tweet_count.toLocaleString()}
                            </Typography>
                          </Box>
                          <Box>
                            <Typography variant="body2" sx={{ color: 'text.secondary' }}>Lists</Typography>
                            <Typography variant="body1">
                              {(selectedAttendee as User & { xProfile?: XUserProfile }).xProfile?.public_metrics.listed_count.toLocaleString()}
                            </Typography>
                          </Box>
                        </Box>
                      </>
                  ...
                  <>
                    ...
                    <Typography variant="subtitle1" sx={{ mb: 1 }}>Joined X</Typography>
                    <Typography variant="body2" sx={{ mb: 3 }}>
                      {new Date((selectedAttendee as any).joinedDate).toLocaleDateString()}
                    </Typography>
                    
                    <Typography variant="subtitle1" sx={{ mb: 1 }}>Interests</Typography>
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 3 }}>
                      {Array.isArray((selectedAttendee as any).interests) && (selectedAttendee as any).interests.map((interest: string) => (
                        <Chip 
                          key={interest} 
                          label={interest} 
                          size="small"
            ...
            {detailTab === 'activity' && (
              <>
                <Typography variant="subtitle1" sx={{ mb: 2 }}>Recent Posts</Typography>
                {activeTab === 'speakers' ? (
                  (selectedAttendee as User & { recentTweets?: XTweet[] }).recentTweets?.map((tweet, index) => (
                    <Paper 
                      key={tweet.id}
                      sx={{ 
                        p: 2, 
                  ...
                  (selectedAttendee as any).recentPosts.map((post: any, index: number) => (
                    <Paper 
                      key={index}
                      sx={{ 
                        p: 2, 
            ...
            {detailTab === 'engagement' && (
              <>
                <Typography variant="subtitle1" sx={{ mb: 2 }}>Engagement History</Typography>
                <Paper 
                  sx={{ 
                    p: 2, 
                    mb: 3, 
                ...
                >
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                    ...
                    <Typography variant="body2">{(selectedAttendee as any).engagement.inSpaceComments}</Typography>
                  </Box>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                    <Typography variant="body2">Liked Posts:</Typography>
                    <Typography variant="body2">{(selectedAttendee as any).engagement.likedPosts}</Typography>
                  </Box>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                    <Typography variant="body2">Recent Interactions:</Typography>
                    <Typography variant="body2">{(selectedAttendee as any).engagement.recentInteractions}</Typography>
                  </Box>
                </Paper>
                
                <Typography variant="subtitle1" sx={{ mb: 2 }}>Message History</Typography>
                <Box sx={{ p: 2, textAlign: 'center' }}>
                  <Typography variant="body2" sx={{ color: 'text.secondary', fontStyle: 'italic', mb: 2 }}>
                    No previous messages with this {activeTab === 'speakers' ? 'speaker' : 'attendee'}
                  </Typography>
            ...
            {detailTab === 'insights' && (
              <>
                <Typography variant="subtitle1" sx={{ mb: 2 }}>AI Insights</Typography>
                <Paper 
                  sx={{ 
                    p: 2, 
                    mb: 3, 
                ...
                >
                  <Typography variant="body2" sx={{ mb: 2, fontStyle: 'italic' }}>
                    Based on this {activeTab === 'speakers' ? 'speaker' : 'attendee'}'s profile and activity, here are some insights:
                  </Typography>
                  
                  <Typography variant="body2" sx={{ mb: 1 }}>
                    • {activeTab === 'speakers' ? 'Key speaker in the space' : 'Highly engaged in AI-related topics'}
                  </Typography>
                  <Typography variant="body2" sx={{ mb: 1 }}>
                    • {activeTab === 'speakers' ? 'Influential voice in the community' : 'Frequently interacts with content about Web3'}
                  </Typography>
                  <Typography variant="body2" sx={{ mb: 1 }}>
                    • {activeTab === 'speakers' ? 'Active in multiple spaces' : 'Active during morning hours (EST)'}
                  </Typography>
                  <Typography variant="body2" sx={{ mb: 2 }}>
                    • {activeTab === 'speakers' ? 'Strong engagement with audience' : 'Responds well to technical deep dives'}
                  </Typography>
                ...
                <Typography variant="subtitle1" sx={{ mb: 2 }}>Engagement Opportunities</Typography>
                <Paper 
                  sx={{ 
                    p: 2, 
                    bgcolor: 'rgba(255, 255, 255, 0.05)',
                ...
                >
                  <Typography variant="body2" sx={{ mb: 2, fontStyle: 'italic' }}>
                    Recommended ways to engage with this {activeTab === 'speakers' ? 'speaker' : 'attendee'}:
                  </Typography>
                  
                  <Typography variant="body2" sx={{ mb: 1 }}>
                    • {activeTab === 'speakers' ? 'Invite to future spaces' : 'Share technical resources about Web3 development'}
                  </Typography>
                  <Typography variant="body2" sx={{ mb: 1 }}>
                    • {activeTab === 'speakers' ? 'Collaborate on content' : 'Invite to upcoming AI Twitter spaces'}
                  </Typography>
                  <Typography variant="body2" sx={{ mb: 2 }}>
                    • {activeTab === 'speakers' ? 'Feature in promotional materials' : 'Mention in a thread about emerging tech trends'}
                  </Typography>
