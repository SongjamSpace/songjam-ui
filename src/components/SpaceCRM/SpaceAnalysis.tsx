  ...
  return (
    ...
    >
      <Handle 
        ...
        id="a"
        style={{ background: 'transparent', border: 'none', width: 1, height: 1, top: '50%', left: '50%' }} 
      />
      <Handle 
        type="target" 
...
const SpaceAnalysis: React.FC<SpaceAnalysisProps> = ({ space, onContextUpdate }) => {
  ...
  const [selectedInteraction, setSelectedInteraction] = useState<SpeakerInteraction | null>(null);
  ...
  useEffect(() => {
    ...
    const simulation = forceSimulation(simulationNodes)
      .force('link', forceLink<ForceNode, ForceEdge>(simulationEdges)
        .id(d => d.id)
        .distance(d => {
  ...
  useEffect(() => {
    ...
    const loadAnalysis = async () => {
      try {
        ...
        // Generate analysis
        const result = await analyzeTranscript(space.spaceId, fullTranscript, space.speakers);
        setAnalysis(result);
  ...
  const generateNetworkGraph = (interactions: SpeakerInteraction[], speakers: User[]) => {
    ...
    const graphEdges: ForceEdge[] = interactions.map((interaction, index) => {
      ...
      if (analysisConfig.strengthMetrics.responseTime && interaction.responseTime) {
        metrics.push(`${Math.round(interaction.responseTime)}s response`);
      }
      
      if (analysisConfig.strengthMetrics.topicOverlap && interaction.topicOverlapScore) {
        metrics.push(`${Math.round(interaction.topicOverlapScore * 100)}% topic overlap`);
      }
    ...
    console.log('Generated initial nodes/edges for D3:', graphNodes, graphEdges);
  ...
  useEffect(() => {
    ...
    const updateAnalysis = async () => {
      try {
        setLoading(true);
        const result = await analyzeTranscript(space.spaceId, transcript, space.speakers, analysisConfig);
        setAnalysis(result);
  ...
  const handleGenerateAnalysis = async () => {
    ...
    try {
      ...
      const result = await analyzeTranscript(space.spaceId, transcript, space.speakers);
      setAnalysis(result);
    ...
    <Paper sx={{ p: 2, mb: 2 }}>
          ...
          <FormControlLabel
            control={
              <Switch
                ...
                onChange={(e) => handleConfigChange(['interactionTypes', 'directMentions'], e.target.checked)}
              />
            }
          ...
          <FormControlLabel
            control={
              <Switch
                ...
                onChange={(e) => handleConfigChange(['interactionTypes', 'sequentialResponses'], e.target.checked)}
              />
            }
          ...
          <FormControlLabel
            control={
              <Switch
                ...
                onChange={(e) => handleConfigChange(['interactionTypes', 'topicBased'], e.target.checked)}
              />
            }
          ...
          <FormControlLabel
            control={
              <Switch
                ...
                onChange={(e) => handleConfigChange(['interactionTypes', 'timeProximity'], e.target.checked)}
              />
            }
          ...
          <FormControlLabel
            control={
              <Switch
                ...
                onChange={(e) => handleConfigChange(['strengthMetrics', 'frequency'], e.target.checked)}
              />
            }
          ...
          <FormControlLabel
            control={
              <Switch
                ...
                onChange={(e) => handleConfigChange(['strengthMetrics', 'duration'], e.target.checked)}
              />
            }
          ...
          <FormControlLabel
            control={
              <Switch
                ...
                onChange={(e) => handleConfigChange(['strengthMetrics', 'topicOverlap'], e.target.checked)}
              />
            }
            label="Topic Overlap"
          />
          <FormControlLabel
            control={
              <Switch
                ...
                onChange={(e) => handleConfigChange(['strengthMetrics', 'sentiment'], e.target.checked)}
              />
            }
          ...
          <FormControlLabel
            control={
              <Switch
                ...
                onChange={(e) => handleConfigChange(['strengthMetrics', 'responseTime'], e.target.checked)}
              />
            }
            label="Response Time"
          />
      ...
      <Box sx={{ mb: 2 }}>
        ...
        <Slider
          ...
          onChange={(_, value) => handleConfigChange(['topicOverlapThreshold'], value as number)}
          min={0}
          max={1}
          step={0.1}
          marks
          ...
          <>
            <Paper sx={{ height: 600, p: 2, mb: 2 }}>
              ...
              <Typography variant="body2" color="text.secondary" gutterBottom>
                Edge thickness represents interaction frequency. Click on any edge to see detailed interaction information.
              </Typography>
            ...
            {currentTopic && (
              <Box>
                <Typography variant="subtitle1">
                  {currentTopic.topic} ({formatTime(currentTopic.startTime)} - {formatTime(currentTopic.endTime)})
                </Typography>
                
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, my: 2 }}>
                  {currentTopic.speakers.map((speakerId) => {
                    const speaker = space.speakers.find(s => s.user_id === speakerId);
                    return speaker ? (
                      <Chip
                        key={speakerId}
        ...
        {tabValue === 2 && (
          <Paper sx={{ p: 2, mb: 2 }}>
            ...
            {analysis.timeline.map((timeline) => {
              const speaker = space.speakers.find(s => s.user_id === timeline.speakerId);
              return speaker ? (
        ...
        <DialogContent>
          {selectedInteraction && space && (
            <Box>
              ...
              {(() => {
                const fromSpeaker = space.speakers.find(s => s.user_id === selectedInteraction.fromSpeakerId);
                const toSpeaker = space.speakers.find(s => s.user_id === selectedInteraction.toSpeakerId);
                return (
                  <Paper sx={{ p: 2, mb: 2 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}>
                        ...
                        />
                        <Typography variant="subtitle1">{fromSpeaker?.display_name}</Typography>
                        <Typography variant="body2" color="text.secondary">@{fromSpeaker?.twitter_screen_name}</Typography>
                      </Box>
                      ...
                      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}>
                        <Avatar 
                          ...
                          sx={{ width: 56, height: 56, mb: 1 }}
                        />
                        <Typography variant="subtitle1">{toSpeaker?.display_name}</Typography>
                        <Typography variant="body2" color="text.secondary">@{toSpeaker?.twitter_screen_name}</Typography>
                      </Box>
                    </Box>
                  </Paper>
                );
              ...
              {/* Interaction Metrics */}
              <Typography variant="h6" gutterBottom>Interaction Metrics</Typography>
              <Paper sx={{ p: 2, mb: 2 }}>
                <Grid container spacing={2}>
                  ...
                  {selectedInteraction.topicOverlapScore !== undefined && (
                    <Grid item xs={6} md={3}>
                      <Box sx={{ textAlign: 'center' }}>
                        <Typography variant="h4" color="primary">
                          {Math.round(selectedInteraction.topicOverlapScore * 100)}%
                        </Typography>
                        <Typography variant="body2" color="text.secondary">
                          Topic Overlap
                        </Typography>
                ...
                <Box sx={{ mt: 2, display: 'flex', alignItems: 'center' }}>
                  <Typography variant="subtitle2" sx={{ mr: 1 }}>Overall Sentiment:</Typography>
                  <Box 
                    component="span" 
                    sx={{ 
              ...
              {/* Topics */}
              <Typography variant="h6" gutterBottom>Discussion Topics</Typography>
              <Paper sx={{ p: 2, mb: 2 }}>
              ...
              {analysis?.timeline && (
                <>
                  <Typography variant="h6" gutterBottom>Example Exchanges</Typography>
                  <Paper sx={{ p: 2, mb: 2 }}>
                    <List>
                      {analysis.timeline
                        .filter(t => 
                        ...
                        .slice(0, 5)
                        .map((segment, index) => {
                          const speaker = space.speakers.find(s => 
