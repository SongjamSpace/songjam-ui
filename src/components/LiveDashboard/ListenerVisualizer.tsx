...
const ListenerVisualizer: React.FC<ListenerVisualizerProps> = ({
  ...
  const graphData = useMemo(() => {
    ...
    for (let i = 0; i < listeners.length - 1; i++) {
      ...
      if (currentListener.joinedAt && nextListener.joinedAt) {
        const timeDiff = Math.abs(currentListener.joinedAt - nextListener.joinedAt);
        // Only create links between listeners who joined within 5 minutes of each other
        if (timeDiff < 300000) {
          links.push({
            source: currentListener.user_id,
            target: nextListener.user_id,
  ...
  const handleNodeClick = useCallback((node: GraphNode) => {
    window.open(`https://twitter.com/${node.user.twitter_screen_name}`, '_blank');
  }, []);
  ...
  if (error) {
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%' }}>
        <Typography color="error">Error loading listeners: {error.toString()}</Typography>
      </Box>
    );
  }
